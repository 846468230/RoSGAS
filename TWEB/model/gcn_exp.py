...
class Net(torch.nn.Module):
	...
	def forward(self, action, datas,all_features,indexes,k_hop_sg,device):
		...
		agg_features = target_feats
		target_feats = self.lin(target_feats)
		return F.log_softmax(target_feats, dim=1),agg_features
	...
	def test(self):
		...
		for act1 in batch_dict.keys():
			...
			for batch in range(num_batches):
				...
				i_end = min((batch + 1) * self.batch_size, length)
				if not self.general:
					logits, aggregated_feature = self.model(act1, batch_dict[act1][i_start:i_end], self.data.x, indexes[i_start:i_end], self.k_hop_sg, self.device)
					batch_label = torch.LongTensor(self.dataset.data.y[torch.LongTensor(indexes[i_start:i_end])]).to(
						self.device)
					aggregated_feature_tmp = aggregated_feature.detach().cpu().numpy()
					if not created:
						aggregated_features = aggregated_feature_tmp
						labels = batch_label.cpu().numpy()
						created = True
					else:
						aggregated_features = np.concatenate((aggregated_features,aggregated_feature_tmp),axis=0)
		...
		# labels = self.dataset.data.y[test_indexes].numpy()
		aggregated_features = np.asarray(aggregated_features)
		...
		return acc,labels,aggregated_features
